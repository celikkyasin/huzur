const {
  ...
  uploadNameImage
} = require("./_lib/namesOfAllahStore");
...
<head>
  ...
  <title>Huzur Admin - 99 Isim Gorselleri</title>
  <style>
...
<body>
  <main>
    ...
    <h1>99 Isim gorselleri</h1>
    <div class="box">
      <h2>Bu alan korumali.</h2>
      <p>Sayfayi admin token ile acman gerekiyor:</p>
...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    ...
    .card{border:1px solid #ded0b8;border-radius:8px;background:#fff;overflow:hidden;cursor:pointer}
  ...
  <script>
    ...
    function render() {
      ...
      for (let i = 1; i <= 99; i += 1) {
        ...
        card.className = "card";
        card.innerHTML =
          '<div class="thumb">' + (item?.imageUrl ? '<img src="' + item.imageUrl + '" alt="">' : 'Gorsel yok') + '</div>' +
    ...
    async function saveImage() {
      try {
        ...
        if (!imageUrl) {
          setStatus("Gorsel linki veya dosya gerekli.");
          return;
        }
        const item = {
          id: "name-" + order,
        ...
        setStatus("Kaydedildi. Uygulama acildiginda guncel gorseli alir.");
      } catch (error) {
...
async function handleUpload(request, response) {
  ...
  const result = await uploadNameImage(payload);
  json(response, result.statusCode, result);
}
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    json(response, 500, {
      ok: false,
      error: "Names of Allah service failed.",
